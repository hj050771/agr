...
public class ClientInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o) throws Exception {
        ...
        if (!httpServletRequest.getRequestURI().contains("/login") && !httpServletRequest.getRequestURI().contains("/register") && !httpServletRequest.getRequestURI().contains("/division") && !httpServletRequest.getRequestURI().contains("/weichat_content")) {
            String authToken = httpServletRequest.getParameter("authToken");
            String loginName = httpServletRequest.getParameter("validParam");
            Accounts accounts = accountsService.selectByLoginName(loginName);
            if (!EncrypAES.encrypt(SHA256Util.getSHA256Str(accounts.getLoginName()), encodeRules).replace("+", "").equals(authToken)) {
